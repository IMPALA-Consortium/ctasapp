...
The app ships with a default `ctasapp.yml` that controls colours, parameter icons,
which ctas features are pre-selected, and (optionally) the location of an
embedded dataset to make available in the Data Source dropdown.

When `run_ctas_app()` is called without an explicit `config = ...`, the
following discovery chain is used:

1. `ctasapp.yml` in the current working directory (i.e. next to `app.R`).
...
— `run_ctas_app()` will discover it automatically via step 1 of the
chain above:
...
embedded:
  ...
  pd: data/pd.parquet                        # optional (protocol deviations)
```
